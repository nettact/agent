...
func (p ifaceTestPlatform) Interfaces() ([]platform.IfaceInfo, error) { return p.ifaces, p.err }
func (p ifaceTestPlatform) WiFi() platform.WiFiResult                 { return p.wifi }
...
func (p ifaceTestPlatform) Supports() []capability.Capability       { return nil }
...
func TestInterfaceCollectorAuthoritativeWiFiSnapshot(t *testing.T) {
	...
	res, err := NewInterfaceCollector(p).Collect(context.Background())
	if err != nil {
...
func TestInterfaceCollectorMissingAdapterAndDisconnectedMetrics(t *testing.T) {
	...
	res, err := NewInterfaceCollector(missing).Collect(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	...
	res, err = NewInterfaceCollector(disconnected).Collect(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	w = res.InterfaceSnapshot.Interfaces[0].WiFi
